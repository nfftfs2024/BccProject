...
sql = """SELECT top 2000 id, deviceid, areaid, LEADAREA, SUBT_DC, SUBSTRING(CONVERT(nvarchar(6),entered_time,112),1,6) entered, entered_time,
...
dfnew = dfnew.reset_index(drop=True)    # Reset indexes for the summary data frame
dfnew['index'] = dfnew.index    # Create a new field saving the current index value
counter = 0     # Reset the counter


# As the previous part get the final weighted flags of the data based on all the data points within its previous and following time intervals
# The timeframe might consist of outliers, which will make some outlier data points valid
# Therefore, the following correction part will take all the data points having wflag = '1' and do the MAD calculation one more time
# which will filter out those latent outliers during the first-round calculation and make the wflags more accurate

### Weighted flag correction ###
for i in range(1, len(sys.argv)-1):
    if (i==1):      # Beginning node, repeated part to be functionized
        dfcor = dfnew[((dfnew.areaid == sys.argv[i]) & (dfnew.next_areaid == sys.argv[i + 1]) & (dfnew.wflag == '1'))]
        # Filter data rows, adding wflag = '1' condition
        dfcor = dfcor.sort_values(['areaid', 'next_areaid', 'entered_time'])    # Sort data by columns
        dfcor = dfcor.reset_index(drop=True)  # Reset index for sorted data rows

        for index, row in enumerate(dfcor.itertuples(), 0):    # Iterate through each row in temporary data frame
            print(counter)
            counter += 1
            ...
            # Extract previous data within the upper bound of time interval and time less than 1 hour for 3 different time intervals
            dflag1 = dfcor[(dfcor['entered_time'] > bottime[0]) & (dfcor['entered_time'] < row.entered_time)]
            # Extract following data within the lower bound of time interval and time less than 1 hour for 3 different time intervals
            dflead1 = dfcor[(dfcor['entered_time'] < uptime[0]) & (dfcor['entered_time'] > row.entered_time)]
            dflag2 = dfcor[(dfcor['entered_time'] > bottime[1]) & (dfcor['entered_time'] < row.entered_time)]
            dflead2 = dfcor[(dfcor['entered_time'] < uptime[1]) & (dfcor['entered_time'] > row.entered_time)]
            dflag3 = dfcor[(dfcor['entered_time'] > bottime[2]) & (dfcor['entered_time'] < row.entered_time)]
            dflead3 = dfcor[(dfcor['entered_time'] < uptime[2]) & (dfcor['entered_time'] > row.entered_time)]
            ...
            for k in range(0, 3):   # Use for loop to assign 3 different time intervals
                dfcor.at[index, lab[k]] = np.where(((row.time >= (medlag[k]-madlag[k]*2)) & (row.time <= (medlag[k]+madlag[k]*2)))
                                                   | ((row.time >= (medlead[k]-madlead[k]*2)) & (row.time <= (medlead[k]+madlead[k]*2))), '1', '0')

            a = [int(x) for x in list((dfcor.loc[index, 'madflag10':'madflag30']))]     # Create a list and convert MAD flags to integers
            dfnew.at[row.index, 'wflag'] = np.where(((sum(np.multiply(level[int(sys.argv[len(sys.argv)-1])-1], a))) >= 0.6), '1', '0')  # Refresh wflag
    # Beginning node wflag correction ends

    elif (i == len(sys.argv)-2):  # Last node
        dfcor = dfnew[((dfnew.areaid == sys.argv[i]) & (dfnew.next_areaid == sys.argv[i - 1]) & (dfnew.wflag == '1'))]  # Filter data rows, adding wflag = '1' condition
        dfcor = dfcor.sort_values(['areaid', 'next_areaid', 'entered_time'])  # Sort data by columns
        dfcor = dfcor.reset_index(drop=True)  # Reset index for sorted data rows
        ...
        for index, row in enumerate(dfcor.itertuples(), 0):  # Iterate through each row in temporary data frame
            ...
            for k in range(0, 3):
                dfcor.at[index, lab[k]] = np.where(
                    ((row.time >= (medlag[k] - madlag[k] * 2)) & (row.time <= (medlag[k] + madlag[k] * 2))) |
                    ((row.time >= (medlead[k] - madlead[k] * 2)) & (row.time <= (medlead[k] + madlead[k] * 2)))
                    , '1', '0')
            a = [int(x) for x in list((dfcor.loc[index, 'madflag10':'madflag30']))]
            dfnew.at[row.index, 'wflag'] = np.where(((sum(np.multiply(level[int(sys.argv[len(sys.argv)-1])-1], a))) >= 0.6), '1', '0')
    # Last node wflag correction ends

    else:  # Middle node
        dire = [sys.argv[i + 1], sys.argv[i - 1]]
        for j in range(0, 2):
            dfcor = dfnew[((dfnew.areaid == sys.argv[i]) & (dfnew.next_areaid == dire[j]) & (dfnew.wflag == '1'))]  # Filter data rows, adding wflag = '1' condition
            dfcor = dfcor.sort_values(['areaid', 'next_areaid', 'entered_time'])  # Sort data by columns
            dfcor = dfcor.reset_index(drop=True)  # Reset index for sorted data rows
            ...
            for index, row in enumerate(dfcor.itertuples(), 0):  # Iterate through each row in temporary data frame
                ...
                for k in range(0, 3):
                    if (madlag[k] == 0) & (madlead[k] == 0):
                        dfcor.at[index, lab[k]] = np.where(((row.time >= (medlag[k] - 2)) & (row.time <= (medlag[k] + 2))) |
                                                            ((row.time >= (medlead[k] - 2)) & (row.time <= (medlead[k] + 2)))
                                                            , '1', '0')
                    else:
                        dfcor.at[index, lab[k]] = np.where(
                ...
                a = [int(x) for x in list((dfcor.loc[index, 'madflag10':'madflag30']))]
                dfnew.at[row.index, 'wflag'] = np.where(((sum(np.multiply(level[int(sys.argv[len(sys.argv)-1])-1], a))) >= 0.6), '1', '0')
    # Middle node wflag correction ends
...
dfnew = dfnew.drop('index',1)   # Drop temp index column
